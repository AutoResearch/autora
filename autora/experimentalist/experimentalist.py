...
import autora.experimentalist.experimentalist_config as exp_cfg
from autora.experimentalist.experiment_environment.experiment_client import (
    Experiment_Client,
)
...
class Experimentalist(ABC):
    def __init__(
        self,
        study_name,
        experiment_server_host=None,
        experiment_server_port=None,
        seed_data_file="",
    ...
    def configure_experiment_client(
        self, experiment_server_host, experiment_server_port
    ):
        self._experiment_server_host = experiment_server_host
        self._experiment_server_port = experiment_server_port
        ...
        for line in file:
            ...
            for IV in object_of_study.independent_variables:
                if IV.get_name() in col_names:
                    ...
                    Exception(
                        'Could not find independent variable "'
                        + IV.get_name()
                        + '" in experiment data file: '
                        + file_path
                    )
            ...
            for DV in object_of_study.dependent_variables:
                if DV.get_name() in col_names:
                    ...
                    Exception(
                        'Could not find dependent variable "'
                        + DV.get_name()
                        + '" in experiment data file: '
                        + file_path
                    )
        ...
        if self._seed_strategy == seed_strategy.UNIFORM:
            ...
            for var in object_of_study.independent_variables:
                factor = Factor(
                    var.get_name(),
                    np.linspace(
                        var._value_range[0], var._value_range[1], resolution
                    ).tolist(),
                )
    ...
    def generate_seed_experiment(self, object_of_study):
        ...
        experiment_file_path = self._write_experiment(
            object_of_study, experiment_sequence
        )
    ...
    def _write_experiment_file(self, object_of_study, experiment_id=None):
        ...
        experiment_file_path = (
            ...
            + AER_cfg.experiment_file_prefix
    ...
    def _write_sequence_file(self, experiment_sequence):
        ...
        try:
            ...
            print(
                "I/O error occured while writing sequence file: " + sequence_file_path
            )
    ...
    def commission_experiment(self, object_of_study, experiment_file_path):
        ...
        if os.path.exists(experiment_file_path):
            experiment_file_destination_path = (
                exp_cfg.client_experiments_path + experiment_file_name
            )
        ...
        if os.path.exists(sequence_file_path):
            sequence_file_destination_path = (
                exp_cfg.client_sequences_path + sequence_file_name
            )
        ...
        client_success = exp_client.submit_job(
            experiment_file_name, clear_sessions=clear_sessions
        )
    ...
    def get_model_fit_plots(self, object_of_study, model):
        ...
        for IV1, IV2, DV in zip(IV_list_1, IV_list_2, DV_list):
            ...
            if self._experiment_sequence is not None:
                x_conditions = list()
                experiment_sequence_rescaled = (
                    object_of_study.rescale_experiment_sequence(
                        self._experiment_sequence
                    )
                )
                if IV2 is None:
                    ...
                    for x1_value, x2_value in zip(
                        experiment_sequence_rescaled[IV1.get_name()],
                        experiment_sequence_rescaled[IV2.get_name()],
                    ):
            ...
            y_limit = [
                np.amin(
                    [np.amin(y_data.numpy()), np.amin(y_prediction.detach().numpy())]
                ),
                np.amax(
                    [np.amax(y_data.numpy()), np.amax(y_prediction.detach().numpy())]
                ),
            ]
            ...
            plot_dict = self._generate_plot_dict(
                ...
                legend=legend,
                image=None,
                x_model=x_prediction.detach().numpy(),
                y_model=y_prediction.detach().numpy(),
    ...
    def sample_experiment(self, model, object_of_study):
        ...
        for condition in self.conditions_per_experiment:
            for i in range(exp_cfg.max_num_rejections):
                trial = self.sample_experiment_condition(
                    model, object_of_study, condition
                )
                if self._experiment_design is not None:
                    valid = self._experiment_design.validate_trial(
                        object_of_study, trial, self._experiment_sequence
                    )
                    if valid:
                        break
                else:
        ...
        experiment_file_path = self._write_experiment(
            object_of_study, self._experiment_sequence
        )
