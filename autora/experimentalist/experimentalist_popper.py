...
import autora.experimentalist.experimentalist_popper_config as popper_config
from autora.experimentalist.experimentalist import Experimentalist
...
class Experimentalist_Popper(Experimentalist, ABC):
    def __init__(
        ...
        super(Experimentalist_Popper, self).__init__(
            study_name,
            experiment_server_host,
    ...
    def init_experiment_search(self, model, object_of_study):
        super(Experimentalist_Popper, self).init_experiment_search(
            model, object_of_study
        )
        ...
        self.popper_pattern_data = np.concatenate(
            (popper_prediction.detach().numpy(), popper_target.detach().numpy()), axis=1
        )
    ...
    def sample_experiment_condition(self, model, object_of_study, condition):
        ...
        probabilities = torch.exp(model_loss * popper_config.optim_beta) / torch.sum(
            torch.exp(model_loss * popper_config.optim_beta)
        )
        ...
        for optimization_epoch in range(self.num_optimization_epochs):
            ...
            with torch.no_grad():
                ...
                for idx in range(len(input_sample)):
                    ...
                    repulsion_from_min = popper_config.limit_repulsion / (
                        dist_to_min**2
                    )
                    repulsion_from_max = popper_config.limit_repulsion / (
                        dist_to_max**2
                    )
                    IV_value_repulsed = (
                ...
                for idx in range(len(input_sample)):
                    ...
                    IV_clipped_value = np.min(
                        [IV_raw_value, np.max(IV_limits) - popper_config.limit_offset]
                    )
                    IV_clipped_value = np.max(
                        [
                            IV_clipped_value,
                            np.min(IV_limits) + popper_config.limit_offset,
                        ]
                    )
        ...
        for idx in range(len(input_sample)):
            ...
            IV_clipped_value = np.max(
                [IV_clipped_value, np.min(IV_limits) + popper_config.limit_offset]
            )
    ...
    def get_plots(self, model, object_of_study):
        ...
        return super(Experimentalist_Popper, self).get_plots(
            object_of_study=object_of_study, model=model
        )
    ...
    def update_popper_pattern_plot(self):
        ...
        # separator
        x = np.ones(int(self.popper_pattern_data.shape[0])) * (
            self.popper_pattern_data.shape[1] / 2 - 0.5
        )
        y = np.linspace(
            1,
            int(self.popper_pattern_data.shape[0]),
            int(self.popper_pattern_data.shape[0]),
        )
