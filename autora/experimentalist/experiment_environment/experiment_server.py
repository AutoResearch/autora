...
class Experiment_Server(Client_Server_Interface):
    ...
    def launch(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as self._s:
            ...
            while True:
                ...
                self._print_status(
                    protocol.STATUS_LISTENING, self.host + ":" + str(self.port)
                )
                ...
                with self._socket:
                    ...
                    while True:
                        ...
                        if data == protocol.CLEAR_SESSIONS:
                            ...
                            self._print_status(
                                protocol.STATUS_CLEARING_SESSIONS,
                            ...
                            self._print_status(
                                protocol.STATUS_RECEIVED_SESSION_ID,
                            ...
                            self._print_status(
                                protocol.STATUS_REQUESTING_EXP,
                            ...
                            while True:
                                ...
                                if data == protocol.TRANSFER_FILEPATH:
                                    ...
                                    file_path = self._socket.recv(1024).decode(
                                        protocol.STRING_FORMAT
                                    )
                                    self._print_status(
                                        protocol.STATUS_RECEIVING_FILE_PATH, file_path
                                    )
                                    self._socket.sendall(protocol.OK)
                            ...
                            self._print_status(
                                protocol.STATUS_COMPLETED_TRANSFER,
    ...
    def _negotiate_status(self):
        ...
        while True:
            ...
            if data == protocol.TRANSFER_CLIENT_STATUS:
                self._print_status(
                    protocol.STATUS_NEGOTIATING_STATUS, "Receiving client status..."
                )
                self._socket.sendall(protocol.OK)
                break
        ...
        self._print_status(
            ...
            "Received job status: " + str(client_status),
        )
        # negotiating status
        ...
        if (
            new_job_status == protocol.JOB_STATUS_REQUESTED_EXPERIMENT
    ...
    def _terminate_session(self):
        self._print_status(
            protocol.STATUS_TERMINATING_SESSION, "Terminating session..."
        )
        self.exp_file_path = ""
        self.seq_file_path = ""
        self.data_file_path = ""
