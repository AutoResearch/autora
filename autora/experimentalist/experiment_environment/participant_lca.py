...
from autora.experimentalist.experiment_environment.participant_in_silico import (
...
class LCA_Model(nn.Module):
    ...
    def forward(self, input):
        ...
        d_x1 = (
            self.input
            - self.gamma * x1
            + self.alpha * torch.relu(x1)
            - self.beta * (torch.relu(x2) + torch.relu(x3))
        ) * self.dt_tau
...
class Participant_LCA(Participant_In_Silico):
    ...
    def __init__(self, input=0, gamma=0.4, alpha=0.2, beta=0.2, dt_tau=1):
        ...
        self.model = LCA_Model(
            input=input, gamma=gamma, alpha=alpha, beta=beta, dt_tau=dt_tau
        )
    ...
    def get_value(self, variable_name):
        ...
        raise Exception(
            'Could not get value from LCA Participant. Variable name "'
            + variable_name
            + '" not found.'
        )
    ...
    def set_value(self, variable_name, value):
        ...
        if variable_name == "x1_lca":
            ...
            raise Exception(
                'Could not set value for LCA Participant. Variable name "'
                + variable_name
                + '" not found.'
            )
    ...
    def figure_plot(
        ...
        (x1_truth_log, x2_truth_log, x3_truth_log) = run_exp(
            ground_truth, x1=x1, x2=x2, x3=x3, n_trials=n_trials
        )
        (x1_approx_log, x2_approx_log, x3_approx_log) = run_exp(
            approximation, x1=x1, x2=x2, x3=x3, n_trials=n_trials
        )
