...
from autora.experimentalist.experiment_environment.experimentalist_GUI import (
    Experimentalist_GUI,
)
from autora.experimentalist.experiment_environment.utils import get_experiment_files
...
class Experiment_GUI(Experimentalist_GUI):
    ...
    def __init__(self, root=None, path=None):
        ...
        self.load_exp_button_style.configure(
            "Load.TButton",
            foreground="black",
        ...
        self.button_load = ttk.Button(
            self._root, text="LOAD", command=self.load_experiment, style="Load.TButton"
        )
    ...
    def init_run(self):
        self.button_run.configure(
            text="RUN EXPERIMENT", command=self.run_experiment, style="Run.TButton"
        )

    def init_STOP_button(self):
        self.button_run.configure(
            text="STOP", command=self.stop_experiment, style="Stop.TButton"
        )
