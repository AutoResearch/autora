...
import autora.experimentalist.experiment_environment.client_server_protocol as protocol
import autora.experimentalist.experiment_environment.experiment_config as config
from autora.experimentalist.experiment_environment.client_server_interface import (
...
class Experiment_Client(Client_Server_Interface):
    def __init__(
        self, session_ID, host=None, port=None, gui=None, experiment_file_name=None
    ):
    ...
    def submit_job(self, experiment_file_name=None, clear_sessions=False):
        ...
        self._print_status(
            protocol.STATUS_CONNECTED,
            "Attempting to connect to " + str(self.host) + ":" + str(self.port),
        )
    ...
    def connect_to_server(self, clear_sessions=False):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as self._socket:
            ...
            self._print_status(
                protocol.STATUS_CONNECTED, "Connected to " + str(self.host) + "."
            )
                    ...
                    if self.job_status == protocol.JOB_STATUS_CONNECTED:
                        ...
                        while True:
                            ...
                            if data == protocol.INITIATE_TRANSFER:
                                self._print_status(
                                    protocol.STATUS_INITIATED_TRANSFER,
                                    "Initiated file transfer.",
                                )
    ...
    def _request_experiment_execution(self):
        self._print_status(
            protocol.STATUS_REQUESTING_EXP, "Requesting experiment execution..."
        )
        ...
        self._print_status(
            protocol.STATUS_REQUESTING_EXP, "Waiting for experiment data..."
        )
    ...
    def _send_experiment_file(self):
        self._print_status(
            protocol.STATUS_INITIATED_TRANSFER, "Sending experiment file: "
        )
    ...
    def _send_sequence_file(self):
        self._print_status(
            protocol.STATUS_INITIATED_TRANSFER, "Sending sequence file..."
        )
    ...
    def _send_session_ID(self):
        self._print_status(
            protocol.STATUS_TRANSFERRING_SESSION_ID, "Sending session ID..."
        )
    ...
    def _negotiate_status(self):
        self._print_status(
            protocol.STATUS_NEGOTIATING_STATUS, "Negotiating job status..."
        )
        ...
        self._print_status(
            protocol.STATUS_NEGOTIATING_STATUS,
            "Sent job status: " + str(self.job_status),
        )
        ...
        while True:
            ...
            if data == protocol.TRANSFER_SERVER_STATUS:
                self._print_status(
                    protocol.STATUS_NEGOTIATING_STATUS, "Receiving server status..."
                )
                self._socket.sendall(protocol.OK)
                break
        ...
        self._print_status(
            protocol.STATUS_NEGOTIATING_STATUS,
            "Negotiated job status:" + str(self.job_status),
        )
