...
import autora.theorist.darts.darts_config as darts_cfg
...
from autora.theorist.darts.utils import format_input_target
from autora.theorist.theorist import Theorist
from autora.utils import Plot_Types
from autora.variable import ValueType as output_types


class Theorist_DARTS(Theorist, ABC):

    simulation_files = "autora/theorist/darts/*.py"
    ...
    def get_model_search_parameters(self):
        ...
        self._model_search_parameters["arch weight decay"] = [
            self.architect.optimizer.param_groups[0]["weight_decay"],
            True,
            lm_float,
        ]
        self._model_search_parameters["arch lr"] = [
            self.architect.optimizer.param_groups[0]["lr"],
            True,
            lm_float,
        ]
        self._model_search_parameters["arch weight decay df"] = [
            self.architect.network_weight_decay_df,
            True,
            lm_float,
        ]  # requires call to architect._init_decay_weights()
        self._model_search_parameters["arch unrolled"] = [
            darts_cfg.unrolled,
            True,
            lm_bool,
        ]
        ...
        # network parameters
        self._model_search_parameters["params momentum"] = [
            self.optimizer.param_groups[0]["momentum"],
            True,
            lm_float,
        ]
        self._model_search_parameters["params weight decay"] = [
            self.optimizer.param_groups[0]["weight_decay"],
            True,
            lm_float,
        ]
        self._model_search_parameters["classifier weight decay"] = [
            self.model._classifier_weight_decay,
            True,
            lm_float,
        ]
        self._model_search_parameters["params max lr"] = [
            darts_cfg.learning_rate,
            False,
            lm_float,
        ]  # self.optimizer.param_groups[0]['lr']
        self._model_search_parameters["params min lr"] = [
            darts_cfg.learning_rate_min,
            False,
            lm_float,
        ]

        # training protocol parameters
        self._model_search_parameters["training set proportion"] = [
            darts_cfg.train_portion,
            False,
            lm_float,
        ]
        self._model_search_parameters["batch size"] = [
            darts_cfg.batch_size,
            False,
            lm_int,
        ]
    ...
    def assign_model_search_parameters(self):
        ...
        # architecture parameters
        self.architect.optimizer.param_groups[0][
            "weight_decay"
        ] = self._model_search_parameters["arch weight decay"][0]
        self.architect.optimizer.param_groups[0]["lr"] = self._model_search_parameters[
            "arch lr"
        ][0]
        self.architect.network_weight_decay_df = self._model_search_parameters[
            "arch weight decay df"
        ][0]
        self.architect._init_decay_weights()
        darts_cfg.unrolled = self._model_search_parameters["arch unrolled"][0]

        # network parameters
        self.optimizer.param_groups[0]["momentum"] = self._model_search_parameters[
            "params momentum"
        ][0]
        self.optimizer.param_groups[0]["weight_decay"] = self._model_search_parameters[
            "params weight decay"
        ][0]
        self.model._classifier_weight_decay = self._model_search_parameters[
            "classifier weight decay"
        ][0]
        self.optimizer.param_groups[0]["lr"] = self._model_search_parameters[
            "params current lr"
        ][0]
    ...
    def get_best_model(self, object_of_study, plot_model=False):
        ...
        for summary_file in self._model_summary_list:
            ...
            if best_loss is None or best_local_log_loss < best_loss:
                ...
                best_num_graph_nodes = int(
                    data[darts_cfg.csv_num_graph_node][min_loss_index]
                )
    ...
    def init_meta_evaluation(self, object_of_study=None):
        ...
        model = self.model
        max_num_architectures = (
            model.max_alphas_normal().shape[0] * model.max_alphas_normal().shape[1]
        )
        n_architectures_sampled = np.min(
            [max_num_architectures, darts_cfg.n_architectures_sampled]
        )
        ...
        self._eval_model_filename_gen = self.get_model_weights_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        self._eval_summary_filename_gen = self.get_model_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        self._eval_arch_filename_gen = self.get_architecture_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
    ...
    def init_model_evaluation(self, object_of_study):
        ...
        self.arch_lr_log = np.empty(
            (self.eval_epochs, 1)
        )  # log architecture learning rate
        self.architecture_weights_log = np.empty(
            (self.eval_epochs, self.num_arch_edges, self.num_arch_ops)
        ...
        logging.info(
            "architecture evaluation for sampled model: %d / %d",
            self._eval_meta_parameters_iteration + 1,
            n_eval_meta_configurations,
        )
        ...
        self._eval_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            ...
            eta_min=darts_cfg.eval_learning_rate_min,
        )
    ...
    def run_eval_epoch(self, epoch, object_of_study):
        ...
        # validation loss
        validation_loss = infer(
            self.valid_queue, self._eval_model, self.criterion, silent=True
        )
    ...
    def _validate_model(self, key):
        if key in self._validation_sets.keys():
            ...
            if (
                output_type == output_types.CLASS
                or output_type == output_types.PROBABILITY_SAMPLE
            ):
    ...
    def log_meta_evaluation(self, object_of_study):
        ...
        # generate header
        header = [
            darts_cfg.csv_theorist_name,
            darts_cfg.csv_model_file_name,
            darts_cfg.csv_arch_file_name,
            darts_cfg.csv_num_graph_node,
            darts_cfg.csv_arch_weight_decay,
            darts_cfg.csv_num_params,
            darts_cfg.csv_num_edges,
            darts_cfg.csv_loss,
        ]
    ...
    def init_model_search(self, object_of_study):
        ...
        split = int(
            np.floor(darts_cfg.train_portion * num_train)
        ...
        self.valid_error_log = np.empty(
            (self.model_search_epochs, 1)
        )  # log validation error
        self.param_lr_log = np.empty(
            (self.model_search_epochs, 1)
        ...
        self.arch_lr_log[:] = np.nan
        self.architecture_weights_log = np.empty(
            (self.model_search_epochs, self.num_arch_edges, self.num_arch_ops)
        )  # log architecture weights
        self.architecture_weights_log[:] = np.nan

        graph_filename = utils.create_output_file_name(
            file_prefix=darts_cfg.graph_filename,
    ...
    def run_model_search_epoch(self, epoch):
        ...
        train_obj = train(
            ...
            self.criterion,
            self.optimizer,
            darts_cfg.arch_updates_per_epoch,
            darts_cfg.param_updates_per_epoch,
        )
    ...
    def log_plot_data(self, epoch, object_of_study):
        ...
        self.prediction_pattern = (
            model_formatted(self.model, input, object_of_study).detach().numpy()
        )
    ...
    def log_model_search(self, object_of_study):
        ...
        model_filename = self.get_model_weights_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        arch_filename = self.get_architecture_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
    ...
    def update_lr_plot(self):
        ...
        if np.isnan(self.param_lr_log[:]).all() and np.isnan(self.arch_lr_log[:]).all():
            ...
            y_max = np.nanmax(
                [np.nanmax(self.param_lr_log[:]), np.nanmax(self.arch_lr_log[:])]
            )
            y_limit = [0, y_max]
        ...
        plot_dict = self._generate_plot_dict(
            type, x, y, x_limit, y_limit, x_label, y_label, legend
        )
    ...
    def update_loss_plot(self):
        ...
        if (
            np.isnan(self.train_error_log[:]).all()
            and np.isnan(self.valid_error_log[:]).all()
        ):
            y_limit = [0, 1]
        else:
            y_max = np.nanmax(
                [np.nanmax(self.train_error_log[:]), np.nanmax(self.valid_error_log[:])]
            )
            y_limit = [0, y_max]
    ...
    def update_model_fit_plots(self, object_of_study):
        ...
        for IV1, IV2, DV in zip(IV_list_1, IV_list_2, DV_list):
            ...
            resolution = int(
                np.round(
                    aer_config.max_data_points_simulated ** (1 / float(n_variables))
                )
            )

            counterbalanced_input = object_of_study.get_counterbalanced_input(
                resolution
            )
            # y_prediction = self.model(counterbalanced_input).detach().numpy()
            y_prediction = (
                model_formatted(self.model, counterbalanced_input, object_of_study)
                .detach()
                .numpy()
            )
            ...
            (input_highlighted, output_highlighted) = object_of_study.get_dataset(
            ...
            if IV2 is None:  # prepare line plot
                ...
                plot_name = (
                    DV.get_name() + "(" + IV1.get_name() + ", " + IV2.get_name() + ")"
                )

                # determine x limits
                x_limit = (
                    object_of_study.get_variable_limits(IV1),
            ...
            plot_dict = self._generate_plot_dict(
                ...
                legend=legend,
                image=None,
                x_model=x_prediction,
                y_model=y_prediction,
                x_highlighted=x_data_highlighted,
        ...
        for edge in range(self.num_arch_edges):
            ...
            for op_idx, operation in enumerate(self.arch_ops_labels):
                ...
                x.append(
                    np.linspace(
                        1,
                        self.architecture_weights_log.shape[0],
                        self.architecture_weights_log.shape[0],
                    )
                )
            ...
            if np.isnan(self.architecture_weights_log[:, edge, :]).all():
                ...
                y_limit = [
                    np.nanmin(np.nanmin(self.architecture_weights_log[:, edge, :])),
    ...
    def update_pattern_plot(self):
        ...
        # generate plot dictionary
        plot_dict = self._generate_plot_dict(
            type, x, y, x_label=x_label, y_label=y_label, image=im
        )
    ...
    def get_model_filename(self, arch_weight_decay_df, num_graph_nodes, seed):
        filename = utils.create_output_file_name(
            file_prefix="model",
            theorist=self.theorist_name,
            log_version=self.model_search_id,
            weight_decay=arch_weight_decay_df,
    ...
    def get_architecture_filename(self, arch_weight_decay_df, num_graph_nodes, seed):
        filename = utils.create_output_file_name(
            file_prefix="architecture_weights",
            theorist=self.theorist_name,
            log_version=self.model_search_id,
            weight_decay=arch_weight_decay_df,
            k=num_graph_nodes,
    ...
    def evaluate_architectures(
        ...
        # generate general model file name
        model_filename_gen = self.get_model_weights_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        summary_filename_gen = self.get_model_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        arch_filename_gen = self.get_architecture_filename(
            arch_weight_decay_df, num_graph_nodes, seed
        )
        ...
        for arch_sample_id in range(n_architectures_sampled):
            ...
            for param_sample_id in range(darts_cfg.n_initializations_sampled):
                ...
                logging.info(
                    "parameter evaluation for sampled model: %d / %d",
                    param_sample_id + 1,
                ...
                scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer,
                    float(darts_cfg.eval_epochs),
                    eta_min=darts_cfg.learning_rate_min,
                )
                ...
                for epoch in range(darts_cfg.eval_epochs):
                    ...
                    nn.utils.clip_grad_norm_(
                        new_model.parameters(), darts_cfg.grad_clip
                    )
                    # moves optimizer one step (applies gradients to weights)
                    optimizer.step()
                    # applies weight decay to classifier weights
                ...
                model_filepath = os.path.join(
                    self.results_weights_path, model_filename + ".pt"
                )
                arch_filepath = os.path.join(
                    self.results_weights_path, arch_filename + ".pt"
                )
                model_graph_filepath = os.path.join(self.results_path, model_filename)
        ...
        # save csv file
        rows = zip(
            model_name_log, arch_name_log, num_graph_node_log, criterion_loss_log
        )
        with open(model_filepath, "w") as f:
            writer = csv.writer(f)
            writer.writerow(
...
def train(
    ...
    for arch_step in range(arch_updates_per_epoch):
        ...
        input_search, target_search = format_input_target(
            input_search, target_search, criterion
        )
...
def init_weights(m):
    if type(m) == nn.Linear:
        ...
        if darts_cfg.init_method == darts_cfg.InitMethod.UNIFORM:
            nn.init.uniform_(
                m.weight,
                a=darts_cfg.init_uniform_interval[0],
                b=darts_cfg.init_uniform_interval[1],
            )
        ...
        # normal initialization
            ...
            nn.init.normal_(
                m.weight, mean=darts_cfg.init_normal_mean, std=darts_cfg.init_normal_std
            )
            if m.bias is not None:
                nn.init.normal_(
                    m.bias,
