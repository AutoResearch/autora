...
def transform_through_primitive_linear(
    x: np.ndarray, a: float = 0.5, b: float = 1.0, seed: int = 0
):
...
def transform_through_primitive_linear_logistic(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
    y = jnp.divide(1, 1 + jnp.exp(-(a * x + b))) + random.normal(
        random.PRNGKey(seed), x.shape
    )
    return y
...
def transform_through_primitive_linear_relu(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
...
def transform_through_primitive_linear_sin(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
...
def transform_through_primitive_linear_cos(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
...
def transform_through_primitive_linear_ln(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
...
def transform_through_primitive_linear_inverse(
    x: np.ndarray, a: float = 0.5, b: float = 1, seed: int = 0
):
...
GENERATORS = {
    ...
    "linear_logistic": lambda x, a, b: transform_through_primitive_linear_logistic(
        x, a, b
    ),
    ...
    "linear_inverse": lambda x, a, b: transform_through_primitive_linear_inverse(
        x, a, b
    ),
}
