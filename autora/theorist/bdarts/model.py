...
class Model(object):
    ...
    def __init__(
        ...
        guide = numpyro.infer.autoguide.AutoNormal(coeff_model, init_loc_fn=loc_fn)
        self.guide = guide
        ...
        self.svi = numpyro.infer.SVI(
            coeff_model, self.guide, self.optimizer, loss=numpyro.infer.Trace_ELBO()
        )
    ...
    def get_posterior_samples(
        ...
        samples = self.guide.sample_posterior(
            random.PRNGKey(1), self.params, (num_samples,)
        )
        prior_predictive = Predictive(coeff_model, samples)
        posterior_samples = prior_predictive(
            random.PRNGKey(1), x, None, arch_params, True, self.priors
        )
