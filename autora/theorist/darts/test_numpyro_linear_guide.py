...
def custom_guide(x, y):
    ...
    b_scale = numpyro.param('b_scale', np.float64(1),
                         constraint=constraints.positive)

    w1_loc = numpyro.param('w1_loc', np.float64(0))
    w1_scale = numpyro.param('w1_scale', np.float64(1),
                            constraint=constraints.positive)

    w2_loc = numpyro.param('w2_loc', np.float64(0))
    w2_scale = numpyro.param('w2_scale', np.float64(1),
                             constraint=constraints.positive)

    a_exp_loc = numpyro.param('a_exp_loc', np.float64(0))
    a_exp_scale = numpyro.param('a_exp_scale', np.float64(1),
                             constraint=constraints.positive)

    a_tanh_loc = numpyro.param('a_tanh_loc', np.float64(0))
    a_tanh_scale = numpyro.param('a_tanh_scale', np.float64(1),
                                constraint=constraints.positive)

    b_exp_loc = numpyro.param('b_exp_loc', np.float64(0))
    b_exp_scale = numpyro.param('b_exp_scale', np.float64(1),
                                constraint=constraints.positive)

    b_tanh_loc = numpyro.param('b_tanh_loc', np.float64(0))
    b_tanh_scale = numpyro.param('b_tanh_scale', np.float64(1),
                                 constraint=constraints.positive)

    sigma_loc = numpyro.param('sigma_loc', np.float64(10),
                                constraint=constraints.positive)
    sigma_scale = numpyro.param('sigma_loc', np.float64(1),
                                constraint=constraints.positive)
