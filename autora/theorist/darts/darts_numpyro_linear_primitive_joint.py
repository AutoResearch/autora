...
def transform_through_primitive_linear_tanh(
    x: np.ndarray, a: float = 0.5, b: float = 1
):
    ...
    for primitive in PRIMITIVES:
        ...
        if num_params > 1:
            coefficients[primitive] = (
                numpyro.sample("a_" + primitive, dist.Normal(0.0, 1.0)),
                numpyro.sample("b_" + primitive, dist.Normal(0.0, 1.0)),
            )
...
for primitive in PRIMITIVES:
    ...
    if num_params > 1:
        coefficients[primitive] = (
            svi_result.params["a_" + primitive + "_auto_loc"],
...
for primitive in PRIMITIVES:
    ...
    if primitive in coefficients.keys():
        ...
        print(
            ...
            + primitive
            + " are "
            + str(coefficients[primitive][0])
            + " and "
            + str(coefficients[primitive][1])
        )
