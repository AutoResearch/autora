...
def model(x, y):
    ...
    w_1 = pyro.sample("w_exp", dist.Normal(-1.0, 1.0))
    w_2 = pyro.sample("w_tanh", dist.Normal(-1.0, 1.0))

    a_exp = pyro.sample("a_exp", dist.Normal(0.0, 1.0))
    a_tanh = pyro.sample("a_tanh", dist.Normal(0.0, 1.0))

    b_exp = pyro.sample("b_exp", dist.Normal(0.0, 1.0))
    b_tanh = pyro.sample("b_tanh", dist.Normal(0.0, 1.0))

    sigma = pyro.sample("sigma", dist.Uniform(0.0, 10.0))
    mean = (
        b
        + softmax(w_1, w_2) * torch.exp(a_exp * x + b_exp)
