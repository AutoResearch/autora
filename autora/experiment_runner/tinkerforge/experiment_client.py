import socket
import experiment_runner.tinkerforge.experiment_config as config
import experiment_runner.tinkerforge.client_server_protocol as protocol
from experiment_runner.tinkerforge.client_server_interface import Client_Server_Interface
from os import path
...
# exp_client.submit_job("experiment101.exp", clear_sessions=True)


def run_experiment(X: np.ndarray, IVs: List[str], DVs: List[str], id: int=1):

    experiment_name = "autora_experiment" + str(id)
    # write sequence file
    # add column to x indexing each row
    X = np.insert(X, 0, np.arange(1, X.shape[0] + 1), axis=1)
    # format X to int
    X = X.astype(int)
    # write X into comma separated file
    np.savetxt(config.client_path + config.sequences_path + experiment_name + "_sequence.csv", X, delimiter=",", fmt='%s', header="trial," + ",".join(IVs), comments='')

    # write experiment file
    exp_file = open(config.client_path + config.experiments_path + experiment_name + ".exp", "w")
    # write IVs
    IVs_str = ",".join(IVs)
    exp_file.write("IV:trial," + IVs_str + "\n")
    # write DVs
    DVs_str = ",".join(DVs)
    exp_file.write("DV:" + DVs_str + "\n")
    # write sequence file name
    exp_file.write("Sequence:" + experiment_name + "_sequence.csv"+ "\n")
    # write data file name
    exp_file.write("Data:" + experiment_name + "_data.csv")
    exp_file.close()
    ...
    import pandas as pd
    data = pd.read_csv(config.client_path + config.data_path + data_file, sep=",")
    return data
