...
parser.add_argument(
    "--topk", type=int, default=5, help="number of best-fitting models listed"
)
parser.add_argument(
    "--winning_architecture_only",
    dest="winning_arch_only",
    action="store_true",
    help="only search models with highest architecture weights",
)
...
(model_name_list, loss_list, BIC_list, AIC_list) = utils.read_log_files(
    resultsPath, args.winning_arch_only
)

# IDENTIFY BEST FITTING MODELS

(topk_loss_names, topk_BIC_names) = utils.get_best_fitting_models(
    model_name_list, loss_list, BIC_list, args.topk
)
