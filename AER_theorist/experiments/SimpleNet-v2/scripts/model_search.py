...
class Network(nn.Module):
  ...
  def __init__(self, num_classes, criterion, steps=2, n_input_states = 2, architecture_fixed = False):
  ...
  def _initialize_alphas(self):
    k = sum(1 for i in range(self._steps) for n in range(2+i)) # the number of possible connections between nodes
  ...
  def genotype(self, sample = False):
    ...
    def _parse(weights):
      ...
      for i in range(self._steps):
        ...
        # first get all the edges for a given node
  ...
  def countParameters(self):
    ...
    return (n_params_total, n_params_base)
