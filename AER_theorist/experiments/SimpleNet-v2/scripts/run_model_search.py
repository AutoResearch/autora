...
parser.add_argument('--draw_samples', type=bool, default=cfg.draw_samples, help='whether to generate probabilistic samples from study object')
parser.add_argument('--log_version', type=int, default=cfg.log_version, help='specify log version')
args = parser.parse_args()

# TEMP ARGS
args.slurm_id = 0
args.object_of_study = cfg.object_of_study
args.draw_samples = cfg.draw_samples
args.log_version = cfg.log_version
...
def main():
  ...
  if(args.debug):
    viz.plot(genotype.normal, graph_filepath)

  # stores the model
  (args.arch_weight_decay_df, args.num_graph_nodes, args.seed)
...
def evaluate_architectures(train_data, train_queue, valid_queue, bic_valid_queue, model, criterion):
  ...
  for sample_id in range(args.n_models_sampled):
      ...
      if args.reinitialize_weights:
          new_model = Network(args.outputDim, criterion, steps=int(args.num_graph_nodes), n_input_states=int(args.inputDim))
      else:
