...
class Theorist_GUI(Frame):
    ...
    # Initialize GUI.
    def __init__(self, theorist, object_of_study, root=None):
        ...
        self.listbox_performance = Listbox(self._root, selectmode=SINGLE, font=(self._font_family, self._font_size),
                                          bg=self._listbox_bgcolor)
        self.listbox_performance.bind('<<ListboxSelect>>', self.update_performance_plot)
        ...
        self.listbox_supplementary = Listbox(self._root, selectmode=SINGLE, font=(self._font_family, self._font_size),
                                           bg=self._listbox_bgcolor)
        self.listbox_supplementary.bind('<<ListboxSelect>>', self.update_supplementary_plot)
    ...
    def update_performance_plot(self, performance_plots):
        from AER_theorist.theorist import Plot_Types

        listbox_selection = self.listbox_performance.curselection()
        if len(listbox_selection) == 0:
            listbox_selection = [0]
        key = str(self.listbox_performance.get(listbox_selection[0]))

        if isinstance(performance_plots, dict) is False:
            performance_plots = self.theorist.get_performance_plots(self.object_of_study)

        if key in performance_plots.keys():
            plot_dict = performance_plots[key]
            ...
            if type == Plot_Types.LINE:
                ...
                # generate plots
                del self._axis_performance_line.lines[:]    # remove previous lines
                plots = list()
                for idx, (x, y, leg) in enumerate(zip(x_data, y_data, legend)):
                    plots.append(self._axis_performance_line.plot(x, y, self._plot_colors[idx], label=leg))

                # adjust axes
                self._axis_performance_line.set_xlim(x_limit[0], x_limit[1])
                self._axis_performance_line.set_ylim(y_limit[0], y_limit[1])

                # set labels
                self._axis_performance_line.set_xlabel(x_label, fontsize=self._plot_fontSize)
                self._axis_performance_line.set_ylabel(y_label, fontsize=self._plot_fontSize)

                self._axis_performance_line.legend(loc=2, fontsize="small")

                self._canvas_performance.draw()
                ...
                # generate image
                self._axis_performance_line.cla()
                self._axis_performance_line.imshow(image, interpolation='nearest', aspect='auto')
                x = x_data
                y = y_data
                self._axis_performance_line.plot(x, y, color='red')
                ...
                plots = list()
                # plot data
                plots.append(self._axis_performance_line.scatter(x_data, y_data, marker='.', c='r', label=legend[0]))

                # plot model prediction
                plots.append(self._axis_performance_line.plot(x_model, y_model, 'k', label=legend[1]))
                ...
                # plot data
                plots.append(self._axis_performance_line.scatter(x1_data, x2_data, y_data, color = (1, 0, 0, 0.5), label=legend[0]))
                # plot model prediction
                plots.append(self._axis_performance_line.plot_surface(x1_model, x2_model, y_model, color=(0, 0, 0, 0.5), label=legend[1]))

                # adjust axes
                self._axis_performance_line.set_xlim(x1_limit[0], x1_limit[1])
                self._axis_performance_line.set_ylim(x2_limit[0], x2_limit[1])
                self._axis_performance_line.set_zlim(y_limit[0], y_limit[1])
            ...
            self._axis_performance_line.set_title(key, fontsize=self._plot_fontSize)
            self._canvas_performance.draw()
    ...
    def run_meta_search(self, resume=False):
        ...
        for idx, meta_params in enumerate(self.theorist._meta_parameters):
            ...
            for epoch in range(self.theorist.model_search_epochs):
                ...
                performance_plots  = self.theorist.get_performance_plots(self.object_of_study)
                self.update_performance_plot(performance_plots)
                self.update_parameter_list(model_search_parameters)
