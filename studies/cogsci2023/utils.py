...
from autora.experimentalist.sampler import nearest_values_sampler, random_sampler, \
    summed_dissimilarity_sampler, model_disagreement_sampler, uncertainty_sampler
...
def fit_theorist(X, y, theorist_name, metadata, theorist_epochs = None):
    ...
    if theorist_name == "BMS" or theorist_name == "BMS Fixed Root":
        ...
        if theorist_epochs is not None:
            ...
            epochs = 500
        theorist = DARTSRegressor(max_epochs=epochs, output_type=output_type, num_graph_nodes=2)
    elif theorist_name == "DARTS 3 Nodes":
        if theorist_epochs is not None:
            epochs = theorist_epochs
        else:
            epochs = 500
        theorist = DARTSRegressor(max_epochs=epochs, output_type=output_type, num_graph_nodes=3)
    elif theorist_name == "DARTS 4 Nodes":
        if theorist_epochs is not None:
            epochs = theorist_epochs
        else:
            epochs = 500
        theorist = DARTSRegressor(max_epochs=epochs, output_type=output_type, num_graph_nodes=4)
...
def get_experimentalist(experimentalist_name, X, y, X_allowed, metadata, theorist, num_samples):
    ...
    # random experimentalist
        ...
        if metadata.dependent_variables[0].type != ValueType.PROBABILITY:
            raise ValueError("Least confident sampling only implemented for probability DVs.")
