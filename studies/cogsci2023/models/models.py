...
model_inventory["stevens_power_law"] = (stevens_power_law_metadata(),
                                    stevens_power_law_data(stevens_power_law_metadata()),
                                    stevens_power_law_experiment)

model_inventory["expected_value"] = (expected_value_theory_metadata(),
                                    expected_value_theory_data(expected_value_theory_metadata()),
                                    expected_value_theory_experiment)

model_inventory["prospect_theory"] = (prospect_theory_metadata(),
                                    prospect_theory_data(prospect_theory_metadata()),
                                    prospect_theory_experiment)

model_inventory["exp_learning"] = (exp_learning_metadata(),
                                    exp_learning_data(exp_learning_metadata()),
                                    exp_learning_experiment)

model_inventory["stroop_model"] = (stroop_model_metadata(),
                                    stroop_model_data(stroop_model_metadata()),
                                    stroop_model_experiment)

model_inventory["task_switching"] = (task_switching_metadata(),
                                    task_switching_data(task_switching_metadata()),
                                    task_switching_experiment)

model_inventory["evc_coged"] = (evc_coged_metadata(),
                                    evc_coged_data(evc_coged_metadata()),
                                    evc_coged_experiment)

model_inventory["evc_demand_selection"] = (evc_demand_metadata(),
                                    evc_demand_data(evc_demand_metadata()),
                                    evc_demand_experiment)

model_inventory["evc_congruency"] = (evc_congruency_metadata(),
                                    evc_congruency_data(evc_congruency_metadata()),
                                    evc_congruency_experiment)

model_inventory["shepard_luce_choice"] = (shepard_luce_metadata(),
                                    shepard_luce_data(shepard_luce_metadata()),
                                    shepard_luce_experiment)

model_inventory["tva"] = (tva_metadata(),
                                    tva_data(tva_metadata()),
                                    tva_experiment)

plot_inventory = dict()
plot_inventory["weber_fechner"] = plot_weber_fechner
plot_inventory["stevens_power_law"] = plot_stevens_power_law
plot_inventory["expected_value"] = plot_expected_value
plot_inventory["prospect_theory"] = plot_prospect_theory
plot_inventory["exp_learning"] = plot_exp_learning
plot_inventory["stroop_model"] = plot_stroop_model
plot_inventory["task_switching"] = plot_task_switching
plot_inventory["evc_coged"] = plot_evc_coged
plot_inventory["evc_demand_selection"] = plot_evc_demand
plot_inventory["evc_congruency"] = plot_evc_congruency
plot_inventory["shepard_luce_choice"] = plot_shepard_luce
plot_inventory["tva"] = plot_tva
