...
class copyNet(nn.Module):
    def __init__(self):
        ...
        self.cell = nn.ReLU(inplace=False)
        # hidden to output projections
        self.classifier = nn.Linear(1, 1, bias=True)

    def init_weights(self):
        self.linear.weight.data.fill_(1)
        # self.linear.bias.data.fill_(-0.18)
        self.classifier.weight.data.fill_(-0.1)
...
# copy_model.linear.weight.data.fill_(1)
# copy_model.linear.bias.data.fill_(-0.18)
# copy_model.classifier.weight.data.fill_(0.5)
# copy_model.classifier.bias.data.fill_(0)
