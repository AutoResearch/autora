...
class ProlificRecruiter:
    ...
    def draft_study(
        ...
        completion_code: str,
        completion_option: CompletionOptions,
    ...
    def update_study(self, study_id: str, **kwargs) -> bool:
        ...
        study = requests.patch(
            f"{self.BASE_URL}studies/{study_id}/", headers=self.HEADERS, json=kwargs
        )
        return study.status_code < 400

    def increase_participant_count(self, study_id: str, new_count: int) -> bool:
        """
        Convenience method pecifically meant for increasing participant count
        """
        return self.update_study(study_id, total_available_places=new_count)

    def publish_study(self, study_id: str, action: StudyAction = StudyAction.PUBLISH):
        """
    ...
    def list_submissions(self, study_id: str) -> Any:
        ...
        submissions = requests.get(
            f"{self.BASE_URL}submissions/", headers=self.HEADERS, params=query
        )
    ...
    def approve_participants(self, study_id: str, prolific_ids: List[str]) -> bool:
        ...
        approve = requests.post(
            f"{self.BASE_URL}submissions/bulk-approve/", headers=self.HEADERS, json=data
        )
