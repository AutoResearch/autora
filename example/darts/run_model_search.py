...
try:
    ...
    import aer.theorist.darts.genotypes as genotypes
    ...
    from aer.theorist.object_of_study import outputTypes
...
parser.add_argument(
    ...
    dest="draw_samples",
    action="store_true",
    help="generate stochastic samples from study object",
)
parser.add_argument(
    "--draw_no_samples",
    dest="draw_samples",
    action="store_false",
    help="generate probabilities from study object",
)
...
(
    args.arch_weight_decay_df,
    args.num_graph_nodes,
    args.seed,
) = utils.assign_slurm_instance(
...
if args.slurm_id == 0:
    utils.create_exp_dir(
        ...
        parent_folder=args.exp_folder,
        results_folder=args.output_file_folder,
    )
...
fh = logging.FileHandler(
    os.path.join(args.exp_folder, args.save, args.output_file_folder, "log.txt")
)
...
def main():
    ...
    # get data
    train_data = args.obj_of_study_class(
        num_patterns=args.num_data_points, sampling=args.draw_samples
    )
    ...
    if args.debug:
        ...
        numArchEdges = model.alphas_normal.data.shape[
            0
        ]  # number of possible edges between nodes (including input nodes)
        ...
        architecture_weights_log = np.empty(
            (args.epochs, numArchEdges, numArchOps)
        ...
        graph_filename = utils.create_output_file_name(
            file_prefix=args.graph_filename,
        ...
        graph_filepath = os.path.join(
            args.exp_folder, args.save, args.output_file_folder, graph_filename
        )

        # plot window
        plotWindow = plotutils.DebugWindow(
            args.epochs,
            numArchEdges,
            numArchOps,
            ArchOpsLabels,
            fitPlot3D=True,
            show_arch_weights=args.show_arch_weights,
        )
    ...
    for epoch in range(args.epochs):  # args.epochs
        ...
        train_obj = train(
            ...
            optimizer,
            lr,
            args.arch_updates_per_epoch,
            args.param_updates_per_epoch,
        )
        ...
        if args.debug:
            ...
            (
                ...
                input_var,
                target_var,
            ) = train_data.sample_model_fit_2d(30, model)

            # compute BIC and AIC
            soft_target = target_var.data.numpy()
            if (
                args.outputType is outputTypes.PROBABILITY_DISTRIBUTION
                or args.outputType is outputTypes.CLASS
            ):
            ...
            # log data
            architecture_weights_log[epoch, :, :] = torch.nn.functional.softmax(
                model.alphas_normal, dim=-1
            ).data.numpy()
...
def evaluate_architectures(
    ...
    model_filename_gen = utils.create_output_file_name(
    ...
    arch_filename_gen = utils.create_output_file_name(
        file_prefix="arch_weights",
        log_version=args.log_version,
        weight_decay=args.arch_weight_decay_df,
        k=args.num_graph_nodes,
        seed=args.seed,
    ...
    for sample_id in range(args.n_models_sampled):
        ...
        for epoch in range(args.epochs):
            ...
            if args.debug:
                ...
                logging.info(
                    "criterion loss %f, BIC %f, AIC %f", criterion_loss, BIC, AIC
                )
        ...
        model_filepath = os.path.join(
            args.exp_folder, args.save, args.output_file_folder, model_filename + ".pt"
        )
        arch_filepath = os.path.join(
            args.exp_folder, args.save, args.output_file_folder, arch_filename + ".pt"
        )
        model_graph_filepath = os.path.join(
            args.exp_folder, args.save, args.output_file_folder, model_filename
        )
    ...
    model_filepath = os.path.join(
        args.exp_folder, args.save, args.output_file_folder, model_filename_csv
    )
...
def train(
    ...
    for arch_step in range(arch_updates_per_epoch):
        ...
        architect.step(
            input_search, target_search, lr, optimizer, unrolled=args.unrolled
        )
