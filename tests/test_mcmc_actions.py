...
def _assert_tree_completeness(tree: Node, depth: int = 0, parent: Node = None):
...
def test_mcmc_de_transform(**hyper_params):
    ...
    node = __build_tree_from_literals(["+", "*", "-", "exp", 0, 1, 2, 3], **hyper_params)
    _assert_tree_completeness(node)
...
def test_mcmc_reassign_op():
    ...
    for _ in range(5):
        ...
        reassign_op(node.left, ops_name_list, ops_weight_list, ops_priors, **hyper_params)
        _assert_tree_completeness(node)
        assert node.left.left.params["feature"] == 0
